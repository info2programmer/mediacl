          ...
          <div class="card-block">
            <table id="advanced-table" class="table dt-responsive table-striped table-bordered nowrap">
              ...
              <tr>
                ...
                <th>Bill Date</th>
                <th>Supplier Name</th>
                <th>Bill Total (wogst)</th>
                <th>Bill Total (wgst)</th>
                <th>Action</th>
              </tr>
              </thead>
              <tfoot>
			...
			$returncnt = $this->db->query('SELECT * FROM td_return_details WHERE inv_no='.$row['p_bill_id'])->num_rows();
			if($returncnt > 0){
			$return = $this->db->query('SELECT SUM(return_price) AS totretprice FROM td_return_details WHERE inv_no='.$row['p_bill_id'])->result_array();
			}
			?>
              <tr>
              	<td><?php echo $i++; ?></td>
                ...
                <td><?php echo $row['p_bill_total']; ?></td> 
                 <td><?php if($returncnt > 0){ echo $row['p_bill_total_sale']-$return[0]['totretprice'];} else {echo $row['p_bill_total_sale'];} ?></td>              
                <td>

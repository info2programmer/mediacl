      ...
      <div class="col-sm-12">
        <div class="card">
          ...
          <div class="card-block">
			...
			$irows = $this->db->query('SELECT * FROM td_purchase_item WHERE item_name="'.$row['stock_item'].'"')->row();
			if(!empty($irows->item_id)){
			$return = $this->db->query('SELECT * FROM td_return_details WHERE item_id='.$irows->item_id)->num_rows();
			}
			?>
              <tr>
              	<td><?php echo $i++; ?></td>
                ...
                <td><?php echo $return; ?> Times Return<?php if(!empty($irows->item_id)){if($return > 0){?><br/><a href="<?php echo base_url();?>View/Returns/<?php echo $irows->item_id; ?>" style="color:#FF0000;">See Return Details</a><?php }} ?></td>              
                <td>

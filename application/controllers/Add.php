...
//$bill_numb = str_pad($number, 8, "0", STR_PAD_LEFT);
		...
		//print_r($this->input->post('txt_media_area'));
			...
			$item_hsn = $this->input->post('item-hsn');
			$item_mrp = $this->input->post('item-mrp');
			...
			$item_p_igst = $this->input->post('item-isgt');
			$item_p_gst = $this->input->post('item-gst');
			$item_p_sgst = $this->input->post('item-ost');
		...
		//$cid = $this->db->insert_id();
			...
			if($service1)
			{
				for($i=0;$i<=$cntExp;$i++){
					...
					$item_s_sing_price_wogst = ($item_p_total[$i]/$item_p_qty[$i]);
					...
					$fields1 = array(
						...
						'item_s_price' => 0,
						'item_p_gst' => $item_p_gst[$i],
						'item_p_sgst' => $item_p_sgst[$i],
						'item_p_igst' => $item_p_igst[$i],
						'item_s_total' => $item_s_price_wgst[$i],
						'item_single_p_wgst' => $item_s_sing_price_wgst,
						'item_single_p_wogst' => $item_s_sing_price_wogst,
						'p_date' => $create_date,
						'pid' => $cid,
...
//echo $lastBill['bill_num'];
			...
			$customer_add = $this->input->post('caddress');
			$customer_phn = $this->input->post('cphn');
			$customer_gstin = $this->input->post('cgstin');
			$expSup = explode('-',$customer_name);
			$ccnt = $this->db->query('SELECT * FROM td_client WHERE clname="'.$expSup[0].'"')->num_rows();
			if($ccnt >0){
				$cust_id = $expSup[1]; 
			} else {
				$fc = array(
					'clname' => $customer_name,
					'clphn' => $customer_phn,
					'cladd' => $customer_add,
					'cgst' => $customer_gstin
					
				);
				$servicefc = $this->base_model->form_post('td_client',$fc);
		...
		//print_r($this->input->post('txt_media_area'));
			...
			$item_s_igst = $this->input->post('item-isgt');
			$item_s_igst_amt = $this->input->post('item-igst-amt');
			$item_s_price_wgst = $item_p_total;
			$item_discount = $this->input->post('item-discnt');
			$item_discount_amt = $this->input->post('item-disc-amt');
			$discount = $this->input->post('discount');
			$main_sale_price = $this->input->post('hid_sourav_distinguish');
			$discount_precentage=$this->input->post('discount-precentage');
			$last_discount_amount=$this->input->post('discount-amt');
		...
		//$expItem = explode(',',$item_name);
			...
			$fields = array(
				...
				'p_bill_total' => $total,
				'stat' => 0,
				'last_discount' => $discount_precentage,
				'last_discount_amount' => $last_discount_amount
			...
			if($service3)
			{
				for($i=0;$i<=$cntExp;$i++){
					...
					$item_s_sing_price_wgst = ($item_s_price_wgst[$i]/$item_p_qty[$i]);
					$expitemname = explode('-',$item_name[$i]);
					$fields1 = array(
						'item_name' => $expitemname[0],
						'item_unit_p_price' => $item_p_price[$i],
						'item_p_qty' => $item_p_qty[$i],
						'itempqtyorig' => $item_p_qty[$i],
						'item_p_unit' => $item_p_unit[$i],

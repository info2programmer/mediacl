...
$this->load->model('base_model');
	...
	public function SalesFinalBill($id)
	{
		...
		$billdatas = $this->db->query('SELECT * FROM td_sales_bill WHERE p_bill_no="'.$id.'"')->result_array();
		$data['billitem'] = $this->db->query('SELECT * FROM td_sales_item WHERE pid='.$billdatas[0]['p_bill_id'])->result_array();
		$data['btotsgst'] = $this->db->query('SELECT SUM(item_s_gst_amt) as totsgst FROM td_sales_item WHERE pid='.$billdatas[0]['p_bill_id'])->result_array();
		$data['btotcgst'] = $this->db->query('SELECT SUM(item_s_cgst_amt) as totcgst FROM td_sales_item WHERE pid='.$billdatas[0]['p_bill_id'])->result_array();
		$data['btotigst'] = $this->db->query('SELECT SUM(item_s_igst_amt) as totigst FROM td_sales_item WHERE pid='.$billdatas[0]['p_bill_id'])->result_array();
		$data['company'] = $this->db->query('SELECT * FROM td_company')->result_array();

...
public function PersonSaleReport()
	{
		...
		$data['slperson'] = $this->db->query('SELECT * FROM td_client ORDER BY clname ASC')->result_array();
		$data['head'] = $this->load->view('elements/head','',true);
		$data['header'] = $this->load->view('elements/header','',true);
		$data['left_sidebar'] = $this->load->view('elements/left-sidebar','',true);
	...
	public function SearchpersonReport()
	{
		...
		$fdate = $this->input->post('fdate');
		$ldate = $this->input->post('tdate');
		$data['rows'] = $this->db->query('SELECT * FROM td_sales_bill WHERE customer_name='.$expEmp[1].' AND p_bill_date BETWEEN "'.$fdate.'" AND "'.$ldate.'"')->result_array();
